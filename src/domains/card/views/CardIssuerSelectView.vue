  ...
  <CardPageLayout bg="screen" @back="returnToCardConnect">
    ...
    <ul class="mt-5 grid grid-cols-2 gap-3" aria-label="카드사 목록">
      <li v-for="issuer in CARD_ISSUER_SELECTION_LIST" :key="issuer.id">
        <button
          type="button"
          class="flex h-16 w-full items-center gap-3 rounded-md border border-primary/30 bg-card px-4 text-left transition-colors hover:border-primary/60 active:bg-accent"
