...
<template>
  <PageLayout title="프로필 수정" has-bottom-bar>
    <form id="profile-edit-form" class="-mt-2" @submit.prevent="saveProfile">
      <div>
        <label for="nickname" class="text-caption mb-2 block font-bold text-muted-text">
          닉네임
        </label>
        <input
          id="nickname"
      ...
      <section class="mt-5" aria-labelledby="connected-account-title">
        <h2 id="connected-account-title" class="text-caption mb-2 font-bold text-muted-text">
          연결된 계정
        </h2>
        <div class="flex min-h-15.75 items-center gap-3 rounded-md bg-screen px-4 py-3.5">
          ...
          <span class="min-w-0 flex-1">
            ...
            <span class="text-micro block font-normal text-muted-text">
              Google 로그인 (변경 불가)
            </span>
          </span>
          <LockKeyhole class="size-3.5 shrink-0 text-muted-text" aria-hidden="true" />
        </div>
      </section>
    </form>
